  ...
  try {
    ...
    try {
      ...
      const leadsByPropertyType = await prisma.buyer.groupBy({
        by: ['propertyType'],
        where: { propertyType: { not: undefined } },
        _count: true,
      ...
      console.log('Sample converted lead data:', JSON.stringify(convertedLeadsData[0], null, 2));
        ...
        for (const lead of convertedLeadsData) {
          if (lead.updatedAt && lead.createdAt) {
            ...
            if (budgetMin !== null && budgetMax !== null) {
              avgBudget = (budgetMin + budgetMax) / 2;
            ...
            console.log(`Lead ${lead.id}: budgetMin=${budgetMin}, budgetMax=${budgetMax}, avgBudget=${avgBudget}`);
